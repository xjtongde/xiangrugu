...
> **状态**：**搁置**（2026-09-23 用户令"现在你不用升级32的，就在36建立我们刚才说的新pg实例。名字就叫pg36"——32 升级缓行，转向 36 新建 pg36，见 `docs/pg36.md`）。**§1 官方核对十项与 §4 Dockerfile 已迁移至 `docs/pg36.md`（R-04 单一源；本页原文存照，不删不回改）**；将来若重启 32 升级，以届时实况重新核证成案。原状态：方案成文（2026-09-23，用户令"制作 pg32 升级方案"），执行待开工口令（R-01）。
