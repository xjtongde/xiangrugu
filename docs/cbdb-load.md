...
- **⑤GiST/栅格清点**：GiST **221**（chgis 162＋harv 59）＝向量 **192** 表＋栅格 **29** 表严丝合缝；带型账：**chgis.dem＝16BSI Int16 真高程**；gtopo30/v3_dem_topo30＝8BUI×3 **Byte RGB 可视化底图**（同点取值 48/48＝通道值非高程，高程真值唯 chgis.dem）；v2_dem 六瓦＝8BUI Byte（中心值 140/76/50/0/204/50）。（**2026-09-27 复测补正**：终态真值＝**222**（chgis 162＋harv **60**）。本项 221 系 b2acc2 清点于 **kozlov 列名归位之前**之快照——同轮 b2acc2 自己数得"空间表=191"（kozlov 几何列时名 `wkb_geometry`、按 geom 口径漏计）、b2post 02:43:47 检出该列、**02:44:26 交互修复**（RENAME→geom＋新建 `kozlov_1899_mk_geom_gix`；修复未入 b2*.py 存档＝记账疏漏，事后亦未重跑⑤）；重装期 GDAL 自建之 `kozlov_1899_mk_wkb_geometry_geom_idx` **随列改名存活且有效** → kozlov 同列现存**两条重复 GiST**（冗余无害：查询择一、写多维护一条）。**表级口径不变**：向量 192 表＋栅格 29 表仍真（几何 GiST 索引 193＝192 表＋kozlov 重 1；栅格 harv 20＋chgis 9＝29 ✓ 实测）。冗余旧名索引是否 DROP 候用户令（R-01）——**2026-09-27 奉令已删**（用户令"如何是多的就要删除掉"）：删前体检＝unique=false／primary=false／valid=true、表主键 `kozlov_1899_mk_pkey`(ogc_fid btree) 不涉；`DROP INDEX harv.kozlov_1899_mk_wkb_geometry_geom_idx` 后复核＝**GiST 221（chgis 162＋harv 59）、几何 192＋栅格 29 表级口径复原、无效索引 0、kozlov 2,351 行不变、EXPLAIN 空间查询走留用 `kozlov_1899_mk_geom_gix` Index Scan、窗口实查 125 命中、全库 395 表（394＋ogr_system_tables）/8,580,012 行不变——完整性零影响，账实复合闭环**。）
