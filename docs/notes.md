...
> **定位（2026-09-22 开设）**：用户交录入档的跨会话纪要之唯一存档处。照录原则：**不改写、不评注、不删减**；纪要内部的 ✅实测／⚠未实测／❌证伪 标注即权威结论，后续会话不得越过复核而改判。新纪要加最上面；编号 `N-01` 起。
> 本账是"存档"不是"需求账"：纪要中可立项的方向条目按 R-02 另挂 `docs/features.md` / `docs/improvements.md`，本账只作其权威展开的单一源（R-04）。
