...
- CBDB《User's Guide》（Fuller 修订版，153 页，PDF 生成 2023-10-06——初录"rev. 2021-10"系误标已补正，见 §12.2 版本注记）：harvard 直连 403（服务器端反爬）→经 web.archive.org/web/20240914131845 取 PDF 全文、本机 `pdftotext` 转文检索（临时文件仅在 /tmp，不入工作区；正式归宿候用户令）；**最新版＝NAS 官方 2025-05 包内自带 160 页版（sha256 `3b703c8e…`，§12.2 改钉补正——账内指南引用已改钉该版）**
