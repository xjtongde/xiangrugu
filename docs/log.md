...
- **2026-09-23 | `docs/codemap.md`（"数据 NAS 挂载"行状态更新）| 改道（写通道激活批）** | 依据：用户于 NAS 端给 admin 开写（B 案）后口令复测。实测坑一条：**服务端改 ACL 后旧 cifs 树连接缓存旧权限，须 umount/mount 重挂生效**；重挂后 root/zky/子目录三写探针全通、探针净清理。上条"待裁决"就此闭环（旧行不回改，本行为指）；guest 可写事实保留为历史记录，不再作为方案选项 | commit `<本批提交后回填>`
