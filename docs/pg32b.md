...
- **cbdb 库扩容**：表 86→**395**（chgis **211**＋harv **97**〔本批新建 schema〕＋public 86＋ogr_system_tables 1）；总行 5,686,842→**≈8,580,012**（＋2,893,170）；库大小 1,543MB→**4,770MB**；GiST **221**（向量 192 表＋栅格 29 表严丝合缝）（**2026-09-27 复测补正：终态真值 222**＝chgis 162＋harv 60——221 系 kozlov 列名归位〔02:44:26〕前快照，kozlov 同列遗留重复 GiST 一条〔冗余无害〕；表级口径 192＋29 不变；详 `docs/cbdb-load.md` §15 验收⑤补正注，冗余索引去留候用户令）；扩展新增 **postgis_raster 3.6.4**；`raster2pgsql` 补装容器 `/usr/local/bin`（镜像原缺，Debian `postgis` 包 `dpkg-deb -x` 提取，RELEASE 3.6.4/ldd 净）。
