...
dump 的生成与调度／备份文件落盘（NAS／~~nas-mirror 大盘~~——**2026-09-27 用户裁决：nas-mirror＝用户自建备份区、项目禁用，pg32b 备份落盘候选剔除之，落点＝NAS 侧或另议**，权威＝`docs/codemap.md` §2 专行）／端口与实例进 homelab 仓登记／pg36 装数联动（F-13 另议）——均另派或另案。
...
- **cbdb 库扩容**：表 86→**395**（chgis **211**＋harv **97**〔本批新建 schema〕＋public 86＋ogr_system_tables 1）；总行 5,686,842→**≈8,580,012**（＋2,893,170）；库大小 1,543MB→**4,770MB**；GiST **221**（向量 192 表＋栅格 29 表严丝合缝）（**2026-09-27 复测补正：终态真值 222**＝chgis 162＋harv 60——221 系 kozlov 列名归位〔02:44:26〕前快照，kozlov 同列遗留重复 GiST 一条〔冗余无害〕；表级口径 192＋29 不变；详 `docs/cbdb-load.md` §15 验收⑤补正注，冗余索引去留候用户令——**同日奉令已 DROP，终值复归 221、账实复合**）；扩展新增 **postgis_raster 3.6.4**；`raster2pgsql` 补装容器 `/usr/local/bin`（镜像原缺，Debian `postgis` 包 `dpkg-deb -x` 提取，RELEASE 3.6.4/ldd 净）。
